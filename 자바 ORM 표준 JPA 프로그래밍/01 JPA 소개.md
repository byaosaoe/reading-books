...
```
- `Member`를 사용할 때마다 `Order`를 함께 사용하면, 이렇게 한 테이블씩 조회하는 것보다 `Member`를 조회하는 시점에 SQL 조인을 사용해서 `Member`와 `Order`를 함께 조회하는 것이 효과적이다.

<hr/>

## 1.2.4 비교
- 객체의 비교는 **동일성**(identity) 비교와 **동등성**(equality) 비교 두 가지 방법이 있다.
...
- ex) 객체를 데이터베이스에 저장할 때 INSERT SQL을 직접 작성하는 것이 아니라 객체를 ORM 프레임워크에 저장하면, ORM 프레임워크가 **적절한 INSERT SQL을 생성**해서 데이터베이스에 객체를 저장해준다.
- 개발자는 ORM 프레임워크에게 객체와 DB 매핑 방법만 알려주면 된다.
- => 객체지향 프로그레밍에 집중 가능
- 자바 진영에서 가장 많이 사용하는 ORM 프레임워크는 **하이버네이트 프레임워크**다.

<hr/>

>
> ## Q & A | ORM에 대한 궁금증과 오해
>   - JPA는 통계 쿼리 같이 복잡한 쿼리보다는 실시간 처리용 쿼리에 더 최적화되어 있다. 상황에 따라 복잡한 통계 쿼리는 SQL을 직접 작성하는 것이 더 쉬운 경우가 많다.
>   - 객체와 SQL을 매핑해주는 SQL 매퍼(마이바티스, 스프링 JdbcTemplate)이 SQL을 직접 작성해야 해서 SQL에 의존하는 개발을 피할 수가 없는 반면, ORM은 SQL에 의존하는 개발을 피할 수 있다.
